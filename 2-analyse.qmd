...
L'objectif de cette partie est de récupérer le temps de transport entre deux gares à partir de leur nom en combinant la fonction `get_travel_time_api_response()` donnée dans la fonction [get_travel_time_api_response()](api.qmd#lst-get_travel_time_api_response) et la fonction [get_station_coordinates()](#lst-get_station_coordinates).
...
get_travel_time_between_stations <- function(station1, station2, data, verbose = TRUE) {
  ...
  if (verbose) {
    message_text <- sprintf("%s -> %s : %s heures\n", station1, station2, ifelse(is.infinite(travel_time), "Aucun itinéraire trouvé", travel_time))
    cat(message_text)
  }
...
travel_time <- get_travel_time_between_stations("Paris-Montparnasse", "Toulouse-Matabiau", stations_data)
travel_time
```
