...
get_travel_time_between_stations <- function(station1, station2, data, verbose = TRUE) {
  ...
  coordinates1 <- get_station_coordinates(station1, data, verbose = FALSE)
  coordinates2 <- get_station_coordinates(station2, data, verbose = FALSE)

  # Générer le JSON pour l'API de routage
  request_body <- get_routes_api_json(coordinates1, coordinates2)
...
air_trafic_df <- read.csv2(AIR_TRAFFIC_DATA_URL)
...
for (pair in city_pairs) {
  air_traffic <- get_air_traffic_between_cities(pair[1], pair[2])
  under_threshold_air_traffic <- under_threshold_air_traffic + air_traffic
}

under_threshold_air_traffic
...
under_threshold_air_traffic * GCO2_PER_PKT / 1000000
```
