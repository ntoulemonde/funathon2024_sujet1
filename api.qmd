...
10. Créez une fonction `get_traveltimeAPI_response()` qui renvoie la réponse de l'API de TravelTime pour un endpoint et une requête donnés. 

```{r}
#| code-fold: true
#| code-summary: Cliquez pour voir la réponse
#| code-overflow: scroll
#| lst-label: lst-get_traveltimeAPI_response
#| lst-cap: get_traveltimeAPI_response

get_traveltimeAPI_response <- function(api_url, request_body) {
...
response_from_function <- get_traveltimeAPI_response(ROUTES_API_URL, request_body)
