...
3. Cliquez sur *Add API Key*, puis copiez la dans votre fichier `secrets.yaml` créé dans lors de l'exercice 1.
...
4. Grâce à l'API de `Stadia Maps` permet de récupérer des fonds de carte avec des styles différents en fonction des besoins et préféreces. En vous aidant de la [documentation de l'API](https://docs.stadiamaps.com/), reconstituez l'URL qui récupère une tuile Stadia Maps pour un style donné.
...
for (i in 1:nrow(emission_by_route_df)) {
  ...
  map <- map %>%
    addPolylines(lat = lat_vector, lng = lng_vector, color = color, opacity = opacity, weight = weight)
}
...
custom_label_options <- labelOptions(noHide = TRUE, style = list("background" = "rgba(255, 255, 255, 0.5)"))
