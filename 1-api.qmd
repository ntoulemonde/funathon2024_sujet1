...
1. Après avoir créé votre compte sur [TravelTime](https://account.traveltime.com/) et récupéré vos identifiants, stockez les dans un fichier `secrets.yaml` comme ci-dessous : 
...
4. Grâce au package `yaml`, importez le contenu du fichier `secrets.yaml` et stocker le contenu dans les variables `X_API_ID` et `X_API_KEY`.
