...
Commencer par créer un compte sur [TravelTime](https://account.traveltime.com/) afin de pouvoir accéder à leur API via des clés API. On créera ensuite un fichier de configuration "config.yaml" structuré comme ci-dessous pour stocker les clés API :
...
# On copie-colle un JSON de test du playground de l'API de routage de TravelTime
json <- '{
  "locations": [
    {
      "id": "point-from",
...
if (!requireNamespace("tmaptools")){install.packages("tmaptools")}
library(tmaptools)
...
# test lille
jsonLilleParis <- getRoutesApiJson(50.63104, 3.046846,43.6044,1.4442)
response2 <- getTraveltimeAPIResponse(ROUTES_API_URL, jsonLilleParis)
