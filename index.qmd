...
Commencer par créer un compte sur [TravelTime](https://account.traveltime.com/) afin de pouvoir accéder à leur API via des clés API. Créer ensuite un fichier de configuration "config.yaml" structuré comme ci-dessous pour stocker les clés API :
...
Pour cela, on crée un JSON (en chaine de caractères) de test pour l'API de routage "routes" de TravelTime à partir de la documentation et du playground disponible [ici](https://playground.traveltime.com/routes). On teste l'API et on observe le contenu de sa réponse.
...
### OPTION 3 : Base de donnée externe des gares

Pour palier au problème de coordonnées induit par les méthodes précédentes on utilise une base de donnée externe avec les coordonnées géographiques des gares françaises qui se situe [ici](https://www.data.gouv.fr/fr/datasets/liste-des-gares/)

```{r}
library(dplyr)

# On charge les données
stationsData <- read.csv2("https://www.data.gouv.fr/fr/datasets/r/d22ba593-90a4-4725-977c-095d1f654d28")
...
*Appeler la fonction 'getStationCoordinates' si on choisi cette option*

```{r}
getCityCoordinates <- function(gare, verbose = TRUE){
  data <- stationsData %>% 
    filter(libelle == gare)
...
On crée une fonction qui prend en paramètre deux couples de coordonnées et qui renvoie un JSON (en chaine de caractères) destiné à l'API de routage

On crée un JSON (en chaine de caractères) pour l'API de routage "routes" de TravelTime à partir de la documentation et du playground disponible [ici](https://playground.traveltime.com/routes) avec les caractéristiques suivantes :
...
getTravelTimeBetweenCities <- function(city1, city2, verbose = TRUE){
  ...
  coordinates1 <- getCityCoordinates(city1, verbose = FALSE)
  coordinates2 <- getCityCoordinates(city2, verbose = FALSE)
  ...
  if (response[[2]] == 429){
    ...
    Sys.sleep(60)
    return(getTravelTimeBetweenCities(city1, city2, verbose))
  }
...
```{r}
travelTime <- getTravelTimeBetweenCities("Toulouse", "Paris")
travelTime
```
