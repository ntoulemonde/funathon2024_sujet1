...
On teste l'envoie d'une requête contenant un JSON (en chaine de caractères) à l'endpoint d'une API de TravelTime grâce à la bibliothèque "httr".

Pour cela, on crée un JSON (en chaine de caractères) pour l'API de routage "routes" de TravelTime à partir de la documentation (playground disponible [ici](https://playground.traveltime.com/routes)). On teste l'API et on observe le contenu de sa réponse.
...
On encapsule le code précédent en une fonction qui envoie une requête contenant un JSON (en chaine de caractères) à l'endpoint d'une API de TravelTime et qui renvoie le contenu et le code de la réponse de l'API
...
troncateJsonList <- function(json, maxLength = 8) {
  if(is.list(json)) {
    # Application récursive pour les sous-listes
    json <- lapply(json, troncateJsonList, maxLength = maxLength)
    # Troncature de la liste avec indication si nécessaire
    if(length(json) > maxLength) {
      json <- json[1:maxLength]
      # Ajout d'un objet spécifique pour indiquer la troncature
      json[[length(json) + 1]] <- list("@LISTE@" = "@TRONQUEE@")
    }
  }
  return(json)
}

# Fonction pour afficher les données JSON proprement
prettyJson <- function(json, maxLength = 8) {
  json <- troncateJsonList(json, maxLength = maxLength)
  return(toJSON(json, pretty = TRUE))
}
```
