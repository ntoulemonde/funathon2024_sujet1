...
#View(response) # Ne marche pas dans Quarto
response
```
...
#View(apiData) # Ne marche pas dans Quarto
apiData
```
...
```{r}
timeTravels <- getTravelTimeBetweenCities("Toulouse", "Lille")
timeTravels
```
...
under2h30Routes <- list()
for (i in 1:length(cities)){
  for (j in 1:i){
    if (i != j){
      if (timeMatrix[cities[i], cities[j]] < 2.5){
        under2h30Routes[[length(under2h30Routes)+1]] <- c(cities[i], cities[j])
      }
    }
  }
}
...
# On crée une fonction qui renvoie le trafic total d'une liaison en 2019 en PKT avec deux noms de villes en paramètres
...
```{r}
under2h30AirTrafic <- 0
for (cities in under2h30Routes){
  airTrafic <- getAirTraficBetweenCities(cities[1], cities[2])
  under2h30AirTrafic <- under2h30AirTrafic + airTrafic
}
under2h30AirTrafic
```

On calcul les émissions de CO2 avec une estimation des gCO2éq par PKT de  [statista](https://www.statista.com/statistics/1113177/co2-emissions-by-airline-europe/)

```{r}
GCO2_PER_PKT <- 80

# On estime les émissions de CO2 en tCO2éq par PKT
under2h30Emissions <- under2h30AirTrafic * GCO2_PER_PKT / 1000000
under2h30Emissions
```
